...
The Rancher Turtles provider images must be locally mirrored as described in the https://documentation.suse.com/cloudnative/cluster-api/latest/en/operator/airgapped.html#_how_to_mirror_capi_provider_oci_artifact[Rancher Turtles Airgap Documentation].  This also requires a turtles.yaml values file for the rancher-turtles-providers chart to specify the necessary configuration.
...
- The `turtles.yaml`: contains the configuration required to specify airgapped operation for Rancher Turtles.
+
[,yaml]
----
TODO: FIXME
...
- A single hostname, resolving to both IPv4 and IPv6, can be used by Metal^3^ to let Ironic use those addresses for binding and URL creation. This approach allows for an easy configuration and flexible behavior (both IPv4 and IPv6 remain viable at each provisioning step), but requires an infrastructure with preexisting DNS servers, IP allocations and records already in place.
