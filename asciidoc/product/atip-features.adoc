...
On x86_64 architectures, the first ACPI C-State is always `POLL`, but it uses a `poll_idle()` function, which may introduce some tiny latency by reading the clock periodically, and restarting the main loop in `do_idle()` after a timeout (this also involves clearing and setting the `TIF_POLL` task flag).
...
The easiest and best way to integrate PTP in your downstream cluster is to add the `linuxptp` package in the Edge Image Builder (EIB) definition file. This way the PTP control plane software will be installed automatically during the cluster provisioning (see https://github.com/suse-edge/edge-image-builder/blob/release-1.0/docs/installing-packages.md[EIB documentation] for more information on installing packages). Below you can find a sample EIB manifest with `linuxptp` included in `packageList`:
...
You can now follow the usual process to build the image as per the EIB documentation (https://github.com/suse-edge/edge-image-builder/blob/release-1.0/docs/building-images.md[]) and use it to deploy your cluster. If you are new to EIB start from xref:../components/edge-image-builder.adoc#components-eib[] instead.

[#ptp-telco-config]
==== Configure PTP for telco deployments

Many telco applications require strict phase and time synchronization with very little deviance, which resulted in definition of two telco oriented profiles: the ITU-T G.8275.1 and ITU-T G.8275.2. They both have a quite high rate of sync messages, as well as other distinctive traits, such as the use of an alternative Best Master Clock Algorithm (BMCA). Such behaviour mandate specific settings in the configuration file consumed by `ptp4l`, provided below as a reference.
...
Once the file has been created, it must be referenced, along with the name of the interface to use for PTP, in order to start the daemon correctly. This can be done by changing the `OPTIONS=` line in `/etc/sysconfig/ptp4l` to:
...
- `$\{INTERFACE_NAME\}` - it will tell `ptp4l` what interface to use for PTP
...
[source,]
----
OPTIONS="-s $INTERFACE_NAME -w"
----

Where `$INTERFACE_NAME` is the name of the interface already set up in ptp4l that will be used as the source for the system clock. This is used, in practice, to identify the source PHC.

[#ptp-capi]
==== Cluster API integration

Whenever a cluster is deployed through directed provisioning and a management cluster, both the configuration file and the two configuration variables in `/etc/systemconfig` can be deployed on the host at provisioning time.
